...
class BusStop {
    ...
    public function deleteStop($id) {
        ...
        try {
            ...
            if ($success) {
                // If everything worked, commit changes
                $this->conn->commit();
                return true;
            } else {
                // If the specific delete failed
                $this->conn->rollback();
                return false;
            }
        } catch (Exception $e) {
